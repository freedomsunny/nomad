...
class BaseAuth(tornado.web.RequestHandler):
    ...
    def get_usermsg_from_keystone(self, token):
        ...
        try:
            ...
            # user_info = utils.get_http(url=options.keystone_endpoint + '/users/bdf1821139284e4393b575257c28f42a',headers=headers)
            # role_info = utils.get_http(url=options.keystone_endpoint+'/tenants/%s/users/%s/roles' % (user_info.json()['user']['tenantId'], user_info.json()['user']['id']), headers=headers)
