...
def get_token(username=options.as_dict().get('keystone_username', ''),
              password=options.as_dict().get('keystone_password', ''),
              tenant=False):
    ...
    if tenant:
        ...
        try:
            ...
            r = post_http(url='%s/tokens' % options.keystone_endpoint, data=json.dumps(data))
            if r.status_code == 200 and r.json().get('access', ''):
